...
__global__ void copy_const_kernel(float *iptr, const float *cptr){
    // map from threadIdx/BlockIdx to pixex position
...
__global__ void blend_kernel(float *outSrc, const float *inSrc){
    ...
    outSrc[offset] = inSrc[offset] + SPEED * (inSrc[top] +  inSrc[bottom] + inSrc[right] + inSrc[left] +4*inSrc[offset]);
...
void anim_gpu(DataBlock *d, int ticks){
    ...
    for(int i = 0; i < 90; i++){
        ...
        blend_kernel<<<blocks,threads>>>(d->dev_constSrc, d->dev_inSrc);
